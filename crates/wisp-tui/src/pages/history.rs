...
impl HistoryPage {
        ...
        let list_items: Vec<ListItem> = self
            ...
            .map(|h| {
                ...
                ListItem::new(Line::from(vec![
                    ...
                    Span::styled(
                        format!("✓{:<4}", h.succeeded),
                        Style::default().fg(Theme::SUCCESS),
                    ),
                    Span::styled(
                        format!("✗{:<4}", h.failed),
                        Style::default().fg(if h.failed > 0 {
                            Theme::DANGER
                        } else {
                            Theme::MUTED
                        }),
    ...
    fn render_detail(&mut self, f: &mut Frame, area: Rect) {
        ...
        let lines = vec![
            ...
            kv_bold(
                "Freed",
                &format_size(entry.bytes_freed, DECIMAL),
                Theme::ACCENT,
            ),
            Line::from(""),
...
fn kv(key: &str, value: &str, value_color: Color) -> Line<'static> {
    Line::from(vec![
        Span::raw("  "),
        Span::styled(format!("{key:<9}"), Style::default().fg(Theme::MUTED)),
...
fn format_age(ts: u64) -> String {
    ...
    if age < 86_400 {
        return format!("{} hr ago", age / 3600);
    }
    if age < 86_400 * 7 {
        return format!("{} days ago", age / 86_400);
    }
    if age < 86_400 * 30 {
        return format!("{} weeks ago", age / (86_400 * 7));
    }
    if age < 86_400 * 365 {
        return format!("{} months ago", age / (86_400 * 30));
    }
