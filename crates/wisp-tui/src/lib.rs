...
use wisp_engine::{Engine, EngineConfig, detect_distro};
...
pub async fn run_tui() -> CoreResult<()> {
    ...
    enable_raw_mode().map_err(wisp_engine::CoreError::Io)?;
    let mut stdout = std::io::stdout();
    execute!(stdout, EnterAlternateScreen).map_err(wisp_engine::CoreError::Io)?;
